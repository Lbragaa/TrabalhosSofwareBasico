...
/* --- Bibliotecas padrão usadas ---------------------------------
   ...
   <stdlib.h>  funções gerais (malloc, exit, etc.)
...
/* Informações completas de um campo, calculadas a partir
   do descritor exemplo "iis03us10" etc */
typedef struct {
    FieldType  type;     /* tipo simbolizado pelo enum acima (para visualizacao mais facil do tipo do campo) */
...
/* Conta quantos campos há no descritor */
static int     count_fields(const char *desc);

/* Converte descritor em vetor dinâmico de FieldInfo */
static int     parse_descriptor(const char *desc,
                                FieldInfo *info,   /* vetor já alocado    */
                                int *nFields,
                                size_t *structSize);

/* Calcula quantos bytes são necessarios pra representar um inteiro  */
...
/* --------------------------------------------------------------------
   ...
   REGRAS:
   • Assumimos que todos os parâmetros são válidos, então a gente não tentou validar nada
   • Só retornamos -1 se ocorrer erro de I/O (fputc, fwrite)
   ------------------------------------------------------------------*/
int gravacomp(int nstructs, void *valores, char *campos, FILE *arq)
{
    /* 1) Descobre quantos campos existem e aloca o vetor dinamicamente aqui  */
    int nF = count_fields(campos);
    FieldInfo *fi = malloc(nF * sizeof(FieldInfo));
    if (!fi) return -1;                       /* falha rara de memória    */

    size_t structSz;
    parse_descriptor(campos, fi, &nF, &structSz);

    /* 2) Escreve quantidade de structs no arqq */
    ...
    for (int s = 0; s < nstructs; ++s) {
        ...
        /* 3a) Percorrer cada campo da struct pelo for                    */
        for (int f = 0; f < nF; ++f) {
            FieldInfo *fld = &fi[f];
            ...
            if (fld->type == FT_STR) {
                ...
                if (fputc(header, arq) == EOF)                 { free(fi); return -1; }
                if (fwrite(str, 1, len, arq) != len)           { free(fi); return -1; }
            }
            ...
            /* ---------- Campo: INTEIRO (signed ou unsigned) --------  */
                ...
                uint32_t v;                                        /* valor */
                /* memcpy evita problema de strict‑aliasing la           */
                ...
                if (fputc(header, arq) == EOF)                { free(fi); return -1; }
                if (fwrite(buf, 1, len, arq) != len)          { free(fi); return -1; }
            }
        } /* fim for campos */
    }     /* fim for structs */
    free(fi);  /* libera vetor de descritores */
...
void mostracomp(FILE *arq)
{
    ...
    printf("Estruturas: %d\n\n", c);
    int remaining = c;                    /* qantas structs faltam exibir */
...
/* --------------------------------------------------------------------
   parse_descriptor
   Converte a string “campos” (ex.: "iis03us10") em um vetor info[], que facilita a compreensao
                            ...
                            size_t *structSize)
{
    ...
    while (*desc) {
        ...
        f.offset = (uint16_t)off; /* salva deslocamento no offset                   */
        off += size;              /* passa pro próximo campo            */
    ...
    /* Ajusta tamanho total da struct para multiplo de maior alinhamento(perfeito) */
...
static size_t int_min_bytes(uint32_t v, int isSigned)
{
    ...
    if (!isSigned) {                /* unsigned: descarta zeros à esquerda */
        ...
        /* eenquanto o byte mais alto for 0 e ainda restarem >1 bytes,
...
/* Conta quantas ocorrências de campo existem na string descritor      */
static int count_fields(const char *d)
{
    int n = 0;
    while (*d) {
        if (*d == 's') d += 3;   /* 's' + 2 dígitos */
        else            d += 1;  /* 'i' ou 'u'      */
        ++n;
    }
    return n;
